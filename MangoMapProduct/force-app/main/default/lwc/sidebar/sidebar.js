...
const columns = [
    { label: 'Standard Object Name', fieldName: 'apiName', hideDefaultActions: "true" }
]
const columnCustom = [{ label: 'Custom Object Name', fieldName: 'apiName', hideDefaultActions: "true" }];
export default class LWCWizard extends LightningElement {
    ...
    @track dateTimeFilterFields = [];
    @track dateFilterFields = [];
    @track NumberFilterFields = [];
    ...
    handleFields() {
        ...
        this.fieldsName.forEach(e => {
            if (e.fieldsName.includes(this.template.querySelector('.fieldsPickList').value)) {
                ...
                } else if (e.datatypeName == 'DATETIME') {
                    if (!this.dateTimeFilterFields.includes(e)) {
                        this.dateTimeFilterFields.push(e);
                        console.log('DateTime: ' + this.dateTimeFilterFields);
                    }
                }
                else if (e.datatypeName == 'DATE') {
                    if (!this.dateFilterFields.includes(e)) {
                        this.dateFilterFields.push(e);
                        console.log('Date: ' + this.dateFilterFields);
                    }
                }
                else if (e.datatypeName == 'NUMBER') {
                    if (!this.NumberFilterFields.includes(e)) {
                        this.NumberFilterFields.push(e);
                        console.log('Number: ' + this.NumberFilterFields);
                    }
                }
    ...
    @track filterDateTimeValue = [];
    @track filterDateValue = [];
    ...
    handleDatetimeFilter(event) {
        console.log('handleDatetimeFilter');
        console.log('value: ' + event.target.value);
        console.log('title: ' + event.target.title);
        console.log('Label: ' + event.target.label);

        console.log(this.tempListDT.findIndex((obj => obj.key == event.target.title)));
        if (this.tempListDT.findIndex((obj => obj.key == event.target.title)) == -1) {
            this.tempListDT.push({ value: [], key: event.target.title });
            console.log(this.tempListDT);
        } else {
            console.log(this.tempListDT);
        }

        var objIndex = this.tempListDT.findIndex((obj => obj.key == event.target.title));
        console.log('objIndex: ' + objIndex);
        console.log(this.tempListDT[objIndex]);
        if (event.target.label == 'From Date') {
            this.tempListDT[objIndex].value[0] = event.target.value;
            console.log(this.tempListDT[objIndex].value[0]);
        } else {
            this.tempListDT[objIndex].value[1] = event.target.value;
            console.log(this.tempListDT[objIndex].value[1]);
        }
        console.log(this.fieldsName);
        var objIndex2 = this.fieldsName.findIndex((obj => obj.fieldApiName == event.target.title));
        console.log('fieldsName objIndex2: ' + objIndex2);
        this.fieldsName[objIndex2].filterValue = this.tempListDT[objIndex].value;
    }
    handleFilter(event) {
    ...
    handleRemovePills(event) {
        console.log('handleRemovePills start');
        console.log(event.target.title);
        console.log(event.target.label);
        console.log(event.target.Name);
        console.log(this.pickListFilterFields);
        var objIndex = this.pickListFilterFields.findIndex((obj => obj.fieldApiName == event.target.title));
        console.log('objIndex: ' + objIndex);
        console.log(this.pickListFilterFields[objIndex]);
        console.log(this.pickListFilterFields[objIndex].filterValue);
        var objIndexFV = this.pickListFilterFields[objIndex].filterValue.findIndex((obj => obj == event.target.label));
        console.log('objIndexFV: ' + objIndexFV);
        console.log(this.pickListFilterFields[objIndex].filterValue[objIndexFV]);
        this.pickListFilterFields[objIndex].filterValue.splice(objIndexFV, 1);
        //this.pickListFilterFields[objIndex].filterValue[objIndexFV] = "";
        console.log(this.pickListFilterFields[objIndex].filterValue);
    }
